...
A single JSON object on stdin::

    {
        "setup": {
            ...
            "location_spec": "Direct",
            "tile_layout": ["Great Mosque", "Post Office", ...]
        },
        "turns": [
...
- ``setup.location_spec`` is optional and defaults to ``"Direct"``.
...
from ..action_json import _CARD_FROM_NAME, _GOOD_FROM_NAME, turn_from_json
...
def game_state_from_json_setup(setup: dict[str, object]) -> GameState:
    ...
    Optional keys: tile_layout (defaults to standard), location_spec (defaults to "Direct").
    """
    ...
    player_hands = {_load_player(p): _CARD_FROM_NAME[c] for p, c in cards_data.items()}
    ...
    if tile_layout_data is not None:
        tile_by_value = {t.value: t for t in Tile}
        tile_mapping: dict[Location, Tile] = {}
        for idx, name in enumerate(tile_layout_data, 1):
            assert name in tile_by_value, f"Unknown tile name: {name!r}"
            tile_mapping[Location(idx)] = tile_by_value[name]
