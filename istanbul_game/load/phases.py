...
class PhaseLoader:
    ...
    @classmethod
    def allowed_cards(cls, gs: GameState, tile: Tile | None = None) -> frozenset[Card]:
    ...
    @classmethod
    def determine_card(cls, card_action: str, gs: GameState, tile: Tile | None = None) -> Card:
        subtokens = action_subtokens(card_action)
        player_state = gs.player_states[gs.turn_state.current_player]
        if subtokens[0].upper() == "CARD":
            cards: set[Card] = {k for k, v in player_state.hand.items() if v > 0}
        else:
            ...
            if action.upper().startswith("CARD"):
                ...
                if card is Card.EXTRA_MOVE:
                    ...
                    yield ExtraMoveCardAction(
                        Move(
                            self.location_transformer.apply(Location(int(subtokens[1]))), skip_assistant=skip_assistant
                        )
                    )
                    if skip_assistant:
                        assert idx == len(actions) - 1, "Assistant skip was not last action"
    ...
    def load_phase_3(self, s: str, tile: Tile | None = None) -> typing.Iterator[PlayerAction]:
        actions = phase_subtokens(s)
        player_state = self.gs.player_states[self.gs.turn_state.current_player]
        ...
        for _idx, action in enumerate(actions):
            ...
            if action.upper().startswith("CARD"):
                ...
                if card is Card.SELL_ANY:
                    ...
                    yield SellAnyCardAction(
                        MarketAction(player_state.cart_contents.copy(), load_good_counter(subtokens[2]))
                    )
                    continue
                yield load_all_phase_card_action(card, subtokens[1:])
                continue
            ...
            if tile is Tile.POLICE_STATION:
                ...
                dest_action = typing.cast(
                    PlaceTileAction | GreenTileAction | DoubleCardAction | SellAnyCardAction, dest_actions[0]
                )
            ...
            if tile is Tile.FOUNTAIN:
                if len(subtokens) == 1 and subtokens[0].upper() == "ALL":
                    ...
                    continue
                locations = map(
                    self.location_transformer.apply, typing.cast(typing.Iterator[Location], map(int, subtokens))
                )
    ...
    def load_governor(self, s: str) -> typing.Iterator[PlayerAction]:
        ...
        for idx, action in enumerate(actions):
            ...
            yield EncounterGovernor(
                load_exact_card(gain), Pay() if cost == "-2" else load_exact_card(cost), load_roll(roll)
            )
    ...
    def load_smuggler(self, s: str) -> typing.Iterator[PlayerAction]:
        ...
        for idx, action in enumerate(actions):
            ...
            if tokens_match(name_subtokens, ["YELLOW", "TILE"]) and len(name_subtokens) == 2:
                ...
                continue
            gain, cost, roll = subtokens
            yield EncounterSmuggler(load_good(gain), Pay() if cost == "-2" else load_good(cost), load_roll(roll))
