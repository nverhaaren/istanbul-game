...
from ..constants import DEFAULT_LOCATIONS, Card, Good, Location, Player, Tile
from ..game import GameState
from ..lib.utils import ImmutableInvertibleMapping
from .core import load_exact_card, load_good_counter, load_player, load_tile
...
class SetupLoader:
    def __init__(self) -> None:
        ...
        self.governor_location: Location | None = None
        self.smuggler_location: Location | None = None
        self.cards: typing.Sequence[Card] = ()

        self._location_spec: str | None = None

    @classmethod
    def load_tiles(cls, tiles: typing.Sequence[str]) -> ImmutableInvertibleMapping[Location, Tile]:
        tile_locations: dict[str, Location] = {s: Location(idx) for idx, s in enumerate(tiles, 1)}
        tile_possibilities: dict[str, set[Tile]] = {s: load_tile(s) for s in tiles}

        final_mapping: dict[Location, Tile] = {}
        possibility_categories: typing.MutableMapping[int, set[str]] = defaultdict(set)
    ...
    def create_phase_loader(self) -> PhaseLoader:
        ...
        location_transformer = LocationTransformer(
            typing.cast(typing.Literal["Roll", "Direct"], self._location_spec), location_map
        )
        ...
        player_hands = dict(zip(self.players, self.cards, strict=True))
