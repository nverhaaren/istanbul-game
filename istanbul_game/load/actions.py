...
from .core import action_subtokens, load_exact_card, load_good, load_good_counter, load_roll, tokens, tokens_match
...
def load_generic_action(s: str) -> GenericTileAction:
    assert not s, f"Action [{s}] is not generic"
    return GenericTileAction()


def load_warehouse_action(s: str) -> GenericTileAction | GreenTileAction:
    if not s:
        return GenericTileAction()
    gt, good = action_subtokens(s)
    assert tokens_match(tokens(gt), ["GREEN", "TILE"]), f"{gt} is not GreenTile"
    return GreenTileAction(load_good(good))


def load_possible_red_tile_action(s: str) -> Roll | RedTileAction:
    subtokens = action_subtokens(s)
    if len(subtokens) == 1:
        return load_roll(subtokens[0])

    first = subtokens[0]
    assert tokens_match(tokens(first), ["RED", "TILE"]), f"{first} is not RedTile"
    assert len(subtokens) == 4, f"RedTile requires 3 inputs, got {len(subtokens) - 1}"
    ...
    method_map: dict[str, typing.Literal["change a die to four", "reroll"]] = {
        "F": "change a die to four",
        "R": "reroll",
        "4": "change a die to four",
    }
...
def load_black_market_action(s: str) -> BlackMarketAction:
    ...
    return BlackMarketAction(
        typing.cast(typing.Literal[Good.RED, Good.YELLOW, Good.GREEN], good), load_possible_red_tile_action(rest)
    )
...
def load_caravansary_action(s: str) -> CaravansaryAction:
    ...
    _Gain = Card | typing.Literal["top of discard pile"]
    first_gain: _Gain
    second_gain: _Gain
...
def load_all_phase_card_action(card: Card, ts: list[str]) -> PlayerAction:
